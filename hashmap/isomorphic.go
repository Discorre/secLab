...
func IsIsomorphic(a, b string) string {
	...
	if len(a) != len(b) {
		return "Не являются изоморфными, так как они имеют разную длину.\n"
	}
	...
	for i := 0; i < len(a); i++ {
		...
		valA, foundA := hashMapA.Get(rune(a[i])) // Получаем соответствующий символ из хеш-таблицы для строки a
		if foundA {
			...
			if valA != rune(b[i]) {
				...
				return "Не являются изоморфными.\n"
			}
		} else {
		...
		valB, foundB := hashMapB.Get(rune(b[i])) // Получаем соответствующий символ из хеш-таблицы для строки b
		if foundB {
