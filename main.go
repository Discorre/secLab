	...
	for {
		...
		case "2":
			...
			for !exit {
				...
				case "size":
					fmt.Println("Текущее количество элементов в множестве:", mySet.Count())
				...
				case "save":
					...
					filename := parts[1]
					err := mySet.WriteToFile(filename)
					if err != nil {
				...
				case "load":
					...
					filename := parts[1]
					err := mySet.ReadFromFile(filename)
					if err != nil {
		...
		case "3":
			...
			for !exit {
				...
				case "partit":
					// Пытаемся разбить множество на подмножества
					if len(parts) < 2 {
						fmt.Println("Недостаточно аргументов для команды partit")
						continue
					}

					targetSum, _ := strconv.Atoi(parts[1])

					// Выполняем разбиение множества на подмножества
					partitions, err := set.Partition(mySet, targetSum)
					if err != nil {
						fmt.Println(err)
					} else {
						fmt.Println("Найденные подмножества:")
						for i := 0; i < 32; i++ {
